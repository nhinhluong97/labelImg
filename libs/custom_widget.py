...
class AngleDialog(QDialog):
    ...
    def get_angle_int(self):
        return int(self.mainLabel.text()) if self.exec_() else None
...
class QCustomQWidget(QWidget):
    ...
    def rotater(self):
        import rotater
        angle_win = AngleDialog()
        angle = angle_win.get_angle_int()
        # reload img
        self.imageData = self.read(self.imagePath, None)
        # img = cv2.imdecode(np.fromstring(self.imageData, np.uint8), cv2.IMREAD_COLOR)
        img = cv2.imdecode(np.fromstring(self.imageData, np.uint8), cv2.IMREAD_COLOR)
        img = rotater.rotate_af_img2(img, angle)
        s = 'save.jpg'
        s = os.path.abspath(s)
        # print(s)
        cv2.imwrite(s, img)  # windows-1252
