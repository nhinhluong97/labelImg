...
class Canvas(QWidget):
    ...
    def mouseMoveEvent(self, ev):
        ...
        isSelectedShape = self.checkSelectedShapeFirst(pos)
    ...
    def checkSelectedShapeFirst(self, pos):
        # Just hovering over the canvas, 2 posibilities:
        # - Highlight shapes
        # - Highlight vertex
        # Update shape/vertex fill and tooltip value accordingly.
    ...
    def selectShapePoint2(self, point):
        """Select the closest shape created which contains this point."""
        self.deSelectShape()
        if self.selectedVertex():  # A vertex is marked for selection.
            index, shape = self.hVertex, self.hShape
            shape.highlightVertex(index, shape.MOVE_VERTEX)
        ...
        for shape in reversed(self.shapes):
            currD = shape.minDistance(point)
            if self.isVisible(shape) and shape.containsPoint(point) and (dist_min == None or dist_min > currD):
                ret_shape = shape
                dist_min = currD
                print(dist_min)

        if ret_shape is not None:
            self.selectShape(ret_shape)
            self.calculateOffsets(ret_shape, point)
        return
    ...
    def callRepaint(self):
        self.repaint()
        return
    def setEditLabel(self):
        self.isEditLabel = True
        return
    def reSetEditLabel(self):
        self.isEditLabel = False
        return
    ...
    def setLastLabel(self, text, subLabels=None, line_color  = None, fill_color = None):
        assert text
        self.shapes[-1].label = text
        self.shapes[-1].subLabels = subLabels
        if line_color:
